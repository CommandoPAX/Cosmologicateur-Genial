...
if __name__ == "__main__" :
    ...
    for i in indices_z:
        ...
        for j in range(len(snapshots)):
                ...
                try :
                    longueurs = np.load(f"/data100/fcastillo/RESULT/{snapshots[j]}/{i}_densite_smooth2_c0.1_surf_murs.txt.npy")
                    ...
                    longueurs = np.load(f"/data100/fcastillo/RESULT/{snapshots[j]}/{k}_densite_smooth2_c0.1_surf_murs.txt.npy")
    ...
    for i in range(len(snapshots)):
        ...
        for j in indices_z:
            ...
            long_lcdm = np.load(f"/data100/fcastillo/RESULT/{snapshots[0]}/{k}_densite_smooth2_c0.1_surf_murs.txt.npy")
            try : long = np.load(f"/data100/fcastillo/RESULT/{snapshots[i]}/{k}_densite_smooth2_c0.1_surf_murs.txt.npy")
            except :     long = np.load(f"/data100/fcastillo/RESULT/{snapshots[i]}/{j}_densite_smooth2_c0.1_surf_murs.txt.npy")
        ...
        for d in range(2):
            ...
            axes.set_xlabel(r"$z$")
            if d == 0 : axs[d].set_ylabel(r"$\langle S \rangle/V~[\mathrm{(Mpc}/h)^{-1}]$")
    ...
    axs[1].invert_xaxis()
    plt.tight_layout()
    plt.savefig(f"surf_moyenne.pdf")
    plt.savefig(f"surf_moyenne.png")
