...
squelette = Squelette_3d(f"/data100/fcastillo/RESULT/{snapshots[N]}/{i}_halos_s1.up.NDskl.S001.a.NDskl")

n = []
for p in squelette.Pointscrit :
    if int(p.type) == 3: n.append(p.nfil)

np.save(f"/data100/fcastillo/RESULT/{snapshots[N]}/{i}_halos_s1_connect_fil.txt", np.array(n))
