            ...
            for d in range(2):
                ...
                for j in range(6):
                    ...
                    if True :#try:
                        ...
                        if d == 0 : 
                            axes.plot(X, count[:,b]/len(count[:,0:3]) , color=couleur, ls=ls,label=label)
                            axes.set_ylabel(r"$\frac{1}{N} \frac{dN}{d\nu}$")
                            axes.xaxis.set_visible(False)
                        else : 
                            axes.plot(X, count[:,b]/len(count[:,0:3])-lcdm[:,b]/len(lcdm[:,0:3]), color=couleur, ls=ls,label=label)
