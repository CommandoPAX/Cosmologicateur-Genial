...
for j in range(4):
    ...
    seuil_haut = np.percentile(field, 90)  # 5% des points les plus hauts
    seuil_bas = np.percentile(field, 10)    # 5% des points les plus bas
    ...
    if True :    densites_interpolees = densites_interpolees[(densites_interpolees <= seuil_bas) | (densites_interpolees >= seuil_haut)]
