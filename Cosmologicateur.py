...
def Power_Spectrum(DATA, index : int, path : str) :
    try :  
        ...
        plt.loglog(k,Pk0*23**3,label="RAMSES") #plot measure from N-body
...
def main(argv):
    ...
    for i in range(1, 10) : 
        ...
        os.system(f'cp -r {input_} "{Output_Path}/ouput_0000{i}"')
        os.system(f'cp -r {ramses_input_} "{Output_Path}/ramses_output_0000{i}"')
